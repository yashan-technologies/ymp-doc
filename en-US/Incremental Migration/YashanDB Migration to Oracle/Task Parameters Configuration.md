Before starting the migration task, a series of task parameter configurations need to be completed. DMC configures these by directly editing the task configuration file. The default task configuration file is the task_config.yml file located in the conf directory.

All parameters supported by DMC and the configuration methods are listed in the [Configuration File Introduction](../DMC Introduction/Configuration Files) document.

### Basic Parameter Configuration

The basic parameter configuration involves making some necessary settings to meet the requirements for successfully starting the task. As follows:

```yaml
taskName: simple_task     # Change to the actual task name
source:
  type: YashanDB             # The source type must be YashanDB
  dataSource:    
    hostname:
...
sink:
  type: oracle            # The target type must be oracle
  dataSource:
    hostname:
      - host: 127.0.0.1     # Change to the actual Oracle server address
        port: 1688          # Change to the actual Oracle listening port
    username: dmc_user      # Change to the actual tool username   
    password: HwfydolzxogxdRxTEmRQyrEq54NceA==           # Fill in the string encrypted by encryptpwd
    database: ORCLCDB      # Oracle Database Service Name, Specify the CDB or PDB name in multi-tenant mode
...
Among them, the database connection passwords for the source and target need to be manually filled in after being encrypted with the DMC client, as follows:
...
#### Data Type Compatibility Parameter

The data type compatibility configuration is only used for automatic table creation in the forward link. In the reverse link, if the metadata coverage strategy is in ignore mode, it may fail due to data type validation.

Configuring data type compatibility may lead to the risk of data synchronization overflow; please use it with caution.

|Data Type Compatibility Configuration |YashanDB Data Type |Oracle Compatible Type |
| -------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------ |
...
| options.dataCompatibility. validateClobMapLong=true      | CLOB                                                         | LONG                                             |
| options.dataCompatibility. validateBlobMapLongRaw        | BLOB                                                         | LONG RAW                                         |

#### Partition Parameter

When the options.partitionTableInterceptionEnabled parameter is set to false, the partition tables in YashanDB will synchronize as ordinary tables.
