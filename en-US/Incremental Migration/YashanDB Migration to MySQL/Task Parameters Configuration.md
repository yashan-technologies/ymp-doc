Before starting the migration task, a series of task parameter configurations need to be completed. DMC configures this by directly editing the task configuration file. The default task configuration file is `task_config.yml`, located in the conf directory.
...
Basic parameter configuration involves making some necessary settings to meet the requirements for the successful startup of a task. As shown below:

```yaml
taskName: simple_task     # Change to the actual task name
source:
  type: YashanDBMySQLMode             # The source type must be YashanDBMySQLMode
  dataSource:    
    hostname:
...
sink:
  type: mysql            # The target type must be mysql
  dataSource:
    hostname:
...
options:
  syncMode: initial              # Change to the actual synchronization mode
...
In this, the database connection passwords for both the source and target sides need to be manually filled in after being encrypted by the DMC client, which can be done as follows:
...
DMC provides a large number of parameters for users to set. Besides the necessary parameter configurations mentioned above, other parameters provide default values to simplify usage.

#### Connector Parameters

Connector parameters offer some optimization configuration options for connecting the source and target sides. Users can set according to their conditions or use the default values.

Detailed descriptions of source and target connector parameters can be found in the [Source Connector Parameter Introduction](../DMC Introduction/Configuration Files.html#connectorparametersource) and [Target Connector Parameter Introduction](../DMC Introduction/Configuration Files.html#connectorparametertarget).

##### Specify Synchronization Starting Point

This specifies the filename of the binary data to be obtained and the starting position for reading the binary data, from which incremental data will be obtained.

The following business scenarios require specifying the synchronization starting point:

- During data migration, if the production environment is busy, to reduce the load on the production environment, a complete sync is performed first, and after the complete sync is done, the incremental starting point is recorded. When the load on the production environment is lighter, the starting point is specified for incremental sync to complete the data sync.
- Syncing only the data generated within a certain time period without needing to start DMC in advance. The starting point for a certain time period is recorded, and when the data is needed, the starting point is specified to start the DMC incremental sync.

YashanDB obtains the starting position for incremental data only from startScn, which can be queried from the database using the following SQL statement:
...
After obtaining the current SCN, a YStream task needs to be manually created to prepare for synchronization. Place the CURRENT_SCN field value into the startScn field of the task configuration file, and then run the script located in the $YDS_HOME/script/createystream directory to generate the SQL statement for creating the YStream task, as follows:

```sql
EXEC DBMS_YSTREAM_ADM.CREATE('taskName','userName', 750069401959915520);
```

For script usage, please refer to the readme_zh.md file in the same directory.

The creation time of the YStream must be earlier than the specified starting point, and the water level point of the YStream must not exceed the specified starting point; otherwise, incremental synchronization will fail.

After the incremental sync ends, the created YStream task can be deleted using one of the following two methods:

1. Delete the startScn field from the task configuration file and then execute the remove command.
2. Execute the following SQL statement in YashanDB:

```sql
EXEC DBMS_YSTREAM_ADM.STOP('taskName');
EXEC DBMS_YSTREAM_ADM.DROP('taskName');
```
...
In MySQL (lower_case_table_names=1), schema and table names are stored in lowercase. To complete data synchronization and data validation, the hidden parameter `options._caseSensitive._object` needs to be configured to lower.
...
DMC supports two types of full incremental synchronization connection methods: **locking synchronization** and **non-locking synchronization**.

- In locking synchronization mode, at the task initialization stage, DMC will pre-apply some connections for locking synchronization. The number of connections requested is related to the metadata synchronization source's concurrency `source.options.metadataParallelism` and full synchronization concurrency `options.snapshotParallelism`, calculated as follows: Number of connections requested = `max(source.options.metadataParallelism, options.snapshotParallelism * 2) + 2`.
- During locking synchronization, please do not perform DDL on objects within the migration scope before the full synchronization stage is completed, as this may lead to task failure. The specific analysis is as follows:
  - Performing DDL operations during the task initialization or metadata synchronization phase may cause metadata validation failures, data synchronization failures, or incremental synchronization failures.
  - Performing DDL operations during the full synchronization stage will not affect the task if CREATE TABLE is executed; otherwise, it will cause data synchronization failure.
- In non-locking synchronization mode, DMC cannot guarantee data consistency, and users must ensure there is no business data writing or DDL changes before the DMC task enters the incremental synchronization phase.
- **DMC strongly recommends using locking synchronization mode**.
...
Data type compatibility configuration is only used for automatic table creation in the forward link. In the reverse link, if the metadata override strategy is set to ignore mode, it may fail due to data type validation.

Configuring data type compatibility may lead to the risk of data synchronization overflow, so please use it cautiously.

|Data Type Compatibility Configuration |YashanDB (mysql mode) Data Types |MySQL Compatible Types |
| -------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------ |
| options.dataCompatibility. allNumericTypeCompatible=true | TINYINT、SMALLINT、INTEGER、BIGINT、NUMBER(P, S)、FLOAT、DOUBLE | NUMBER(P, S)、BINARY_FLOAT、BINARY_DOUBLE、FLOAT |
| options.dataCompatibility. validateStringLength=false    | VARCHAR(size)                                                | VARCHAR(any size)                                |
| options.dataCompatibility. validateSmallintMapYear=true  | SMALLINT                                                     | YEAR                                             |
| options.dataCompatibility. validateVarcharMapEnum=true   | VARCHAR(size)                                                | ENUM                                             |
| options.dataCompatibility. validateVarcharMapSet=true    | VARCHAR(size)                                                | SET                                              |

#### JDBC Connection Parameters

During the full stage, the JDBC connection to the MySQL target side defaults to having `rewriteBatchedStatements` set to true, while in the incremental stage it defaults to false. Users can set the `sink.dataSource.properties.rewriteBatchedStatements` parameter in the task configuration file to override the default value, which will then take effect in both full and incremental stages.

When `rewriteBatchedStatements` is set to true, it will not allow accurate statistics of the number of affected synchronized rows from the batch execution results returned after executing `executeBatch()` from MySQL JDBC. This may lead to inaccurate results for DML checks enabled by `sink.options.checkDml`. Therefore, in either initial or increment mode, `rewriteBatchedStatements` and `sink.options.checkDml` cannot both be set to true.
