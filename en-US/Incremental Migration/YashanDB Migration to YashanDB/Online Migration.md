...
The metadata synchronization phase will run tasks to synchronize the Schema and tables (excluding system tables and temporary tables) within the configured synchronization scope. The table synchronization scope is as follows:
...
If full fault tolerance is enabled, when a table creation fails, that table will be fault-tolerated while other tables continue to synchronize.

#### Metadata Validation

During the metadata synchronization phase, a metadata validation will be performed to ensure that the metadata of the tables on the source and target ends meet synchronization requirements. The validation includes:
...
- The column types of corresponding columns in the source and target are YashanDB compatible types.
...
The synchronization scope of incremental DDL is shown in the following table:

|DDL Type |Synchronization Scope |Example |
| -------------------------------------------------- | -------------------------- | ------------------------------------------------------------ |
| CREATE TABLE                                      | Column name, column type, default value, primary key | CREATE TABLE YDS.TEST(ID INT DEFAULT 1 PRIMARY KEY)        |
...
| ALTER TABLE ADD COLUMN                            | Column name, column type, default value, primary key | ALTER TABLE YDS.TEST ADD COLUMN COL0 INT DEFAULT 1 PRIMARY KEY |
...
| ALTER TABLE MODIFY                                | Column type, default value, primary key     | ALTER TABLE YDS.TEST MODIFY (ID INT DEFAULT 1 PRIMARY KEY)  |
...
| ALTER TABLE DROP CONSTRAINT primary_key_constraint | -                             | ALTER TABLE YDS.TEST DROP CONSTRAINT PK (the constraint named `PK` is the primary key constraint)  |

> **Caution**：
>
> In the above DDL, non-primary key indexes, foreign keys, partition keys, and other constraints and parameters related to tables (such as PCTFREE, etc.) will not be synchronized.

If fault tolerance for incremental DDL is configured in the task parameter file, you can execute `show faultTolerance` on the client to view the currently fault-tolerated tables and related information:

![](./image/ddl_fault_tolerance.png)

When the DDL `DROP TABLE YDS.NOT_SUPPORTED_TABLE` is captured, the table `YDS.NOT_SUPPORTED_TABLE` will no longer be fault-tolerated, and DMC will attempt to delete that table on the target end, and subsequently capture the DDL and DML for that table.
