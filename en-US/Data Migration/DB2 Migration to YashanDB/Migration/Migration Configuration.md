...
- **[ Compatibility Configuration ]**: This project is used to configure compatibility processing for NULL data, with no compatibility applied by default.

  For source data that the target side considers illegal, YMP first converts it to NULL before executing the insertion. When compatibility configuration is selected, YMP will also attempt to remove not-null constraints before insertion to avoid migration failure due to constraint violations.

  > **Note**:
  >
  > When the version of YashanDB on the target side is greater than or equal to 23.3, and the database parameter EMPTY_STRING_AS_NULL is set to FALSE, YMP will insert empty strings of CHAR and VARCHAR types from DB2 directly into YashanDB; other types of empty strings will be converted to NULL before insertion. 

- **[ Enable Statistics Collection ]**: This project is used to configure whether to collect statistics on the target side during the migration process.

  Enabling statistics collection may increase migration time. When statistics collection is not enabled, if there are a large number of changes in metadata, this may lead to invalid statistics on the target side, requiring manual collection after migration to improve SQL execution efficiency.

- **[ Data Migration Export Tool ]**: This project is used to configure the export tool during migration. When choosing DTS mode, the following limitations apply:

  - Due to limitations in YASLDR version 22.2, data migration in DTS mode currently does not support passwords containing the "@" character.

  - When migrating TIME ZONE types, DTS cannot be chosen, which may lead to incorrect migration results.
  - When migrating BFILE types, DTS cannot be chosen, which may lead to migration failure.
