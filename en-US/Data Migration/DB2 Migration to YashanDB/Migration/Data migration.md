The data migration phase mainly implements the migration of metadata and data. Before migration, multiple checks will be performed, and upon passing the checks, the migration will begin. Users can view the migration progress and details in real time.

The following general rules apply to YMP's data migration:

- Support for pausing migration tasks during operation, resuming migration after pausing, terminating migration after pausing, and displaying current progress information after pausing.
- For newly created users (Schemas) on the target side by the migration task, the user (Schema) will be granted both CONNECT and RESOURCE privileges.

- After migration completion, the default password for the DBLINK object will be set to yasdb_123.

  - If there are completely empty rows (discard rows) on the source side causing the migration to fail, the data file will not be retained. Information about the failure due to empty rows can be found in the discard.log file located in the same directory as the data file (YMP installation directory/tmp/).

  - If the schema used for migration tasks in the beginning step is already being used by other tasks in the selected target database, a prompt will appear during saving or proceeding to the next step indicating that [xxxx] is already used in task [yyyy] on the target side, please delete the related tasks and recreate. Here, xxxx is the Schema name involved, and yyyy is the task name involved.

  - Migration for schema names or table names with single quotes is not supported.
