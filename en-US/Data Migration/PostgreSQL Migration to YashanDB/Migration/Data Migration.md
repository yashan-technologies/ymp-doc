The data migration phase mainly implements the migration of metadata and data. Before the migration, several checks will be conducted. Once the checks are passed, the migration begins, and users can view the migration progress and details in real time.

The following general rules apply to YMP's data migration:

- It supports pausing a migration task during its operation, resuming the migration after pausing, terminating the migration after pausing, and displaying the current progress information after pausing.
- For users (Schemas) newly created on the target side by the migration task, the user (Schema) will be granted both CONNECT and RESOURCE privileges.

- After migration is completed, the default password for the DBLINK object will be set to yasdb_123.

  - If there are all-empty rows (discard rows) on the source side that cause the migration to fail, the data file will not be retained. Information about the failure of all-empty row migration can be found in discard.log in the same directory as the data file (YMP installation directory/tmp/).

  - If the schema used by the migration task is being used by another task in the selected target database, a prompt will appear when saving or moving to the next step: [xxxx] is already in use at the target side of task [yyyy], please delete the related task and re-create it. Here, xxxx refers to the involved Schema name, and yyyy refers to the involved task name.

  - Migration of Schemas or table names containing single quotes is not supported.
