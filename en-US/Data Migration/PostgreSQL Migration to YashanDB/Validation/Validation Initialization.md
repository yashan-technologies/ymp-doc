...
* Performance Configuration: Used to configure the maximum number of table checks that can be performed simultaneously. Each table check will use one thread, with one database connection for the source end and one for the target end. The default value is 50, and the range of values is [1,200].

### Table Check Configuration

This section will list all the table-level verification task configurations provided by YMP. For verification tasks on the PostgreSQL source end, the following items can be configured:

* **[ Single Table Inconsistency Threshold ]**: You can select unlimited or enter a custom threshold. Verification tasks will be aborted once the threshold is reached.
* **[ Character Type Options ]**: Trimming of right-side spaces for CHAR data type is enabled by default; trimming of right-side spaces for VARCHAR data type is disabled by default.
* **[ Numeric Type Options ]**: The default precision for FLOAT data type verification is 6, with a range of [1,10]; the default precision for DOUBLE data type verification is 15, with a range of [1,20].
* **[ Enable Periodic Validation ]**: Periodic checks can be configured by setting specific times and cron expressions, allowing tasks to perform periodic verification. It supports two types of periodic checks: regular verification or flashback verification.
