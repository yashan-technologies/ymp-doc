...
## Source MySQL Authorization

To ensure a smooth migration, please ensure that the source MySQL user has the following privileges:
...
-- Additional privilege required for MySQL 8.0:
...
GRANT SHOW DATABASES ON *.* TO 'yourUser'@'IP';
GRANT SELECT ON *.* TO 'yourUser'@'IP';
GRANT PROCESS ON *.* TO 'yourUser'@'IP';  -- Suggested privilege, not required. If MySQL full verification termination functionality is needed, it is a necessary privilege.
```
...
> **Note**：
>
> The `yourUser` and `IP` in MySQL authorization refer to the user information specified when creating the user, as referenced in `CREATE USER 'newUser'@'IP' IDENTIFIED BY 'password'`. Here `yourUser=newUser` and `IP=% or xx.xx.xx.xx`.
...
== YashanDB (Enable Three Powers Separation)
