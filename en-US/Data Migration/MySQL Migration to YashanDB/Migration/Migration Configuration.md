...
* Support renaming Schema (i.e., configuring a target Schema with a different name from the source Schema). The input name cannot exceed 64 characters and can only include English letters, numbers, and the special character _.

  If there are Event objects under the Schema to be renamed, due to syntactic analysis limitations, these objects may cause migration failures because the Schema name cannot be changed during the migration process.
...
\<shardoc fromid=migrate_config_Compatibility>

> **Note**:
>
> For MySQL source, character types other than CHAR and VARCHAR that contain empty string data must not choose DTS; otherwise, the empty string data will be converted to NULL during migration.
