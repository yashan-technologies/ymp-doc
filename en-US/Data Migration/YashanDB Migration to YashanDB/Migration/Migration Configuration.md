...
> **Note**:
>
> If you want to adopt a non-persistent migration method, you can modify the configuration **[ YashanDB Source Available Migration Mode ]** to non-persistent mode.
...
* Support renaming of Schema (i.e., configuring a target Schema with a different name on the source side). The entered name cannot exceed 64 characters and can only include English letters, numbers, and the special character _. 

  If there are COMPRESSION tables, AC constraints, scheduled tasks, or DBLINKs under the Schema to be renamed, the migration may fail due to the inability to change the Schema name, which is a limitation of syntax analysis capability.
