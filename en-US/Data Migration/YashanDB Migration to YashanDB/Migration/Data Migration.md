The Data migration phase mainly implements the migration of metadata and data. Multiple checks will be performed before migration. After passing the checks, the migration will begin, and users can view the migration progress and details in real time.

The following general rules exist for YMP's data migration:

- It supports pausing migration tasks during execution, resuming migration after pausing, terminating migration after pausing, and displaying current progress information after pausing.
...
- After migration is complete, the default password for the DBLINK object will be set to yasdb_123.

  - If the source side has empty rows (discard rows) that cause migration failure, the data file will not be retained. The failure information related to empty row migration can be found in the discard.log located in the same directory as the data file (YMP installation directory/tmp/).

  - If the Schema used in the migration task at the beginning step is already used by another task in the selected target database, a prompt will appear when saving or proceeding to the next step: [xxxx] is already used in task [yyyy] on the target side, please delete the relevant task and recreate. Here, xxxx is the name of the involved Schema, and yyyy is the name of the involved task.

  - Migration of Schema names or table names containing single quotes is not supported.
  - Migration of BIT types in YashanDB is not supported.

> **Note**:
>
> After the migration task ends, you can modify the configuration **[ YashanDB Source Available Migration Mode ]** to non-persistence mode. Click to re-migrate, and the task will execute migration in non-persistence mode. 
