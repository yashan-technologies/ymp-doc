...
* Performance Configuration: This is used to configure the maximum number of tables that can be validated simultaneously. Each validation of a table will use one thread, while both the source and target sides each occupy one database connection. The default value is 50, and the range is [1,200].

### Table Validation Configuration

This section will list all table-level validation task configurations provided by YMP. For validation tasks on Oracle source ends, the following items can be configured:

* **[ Timestamp Precision ]**: Set the precision value for validating the decimal point of timestamp, with a range of [0,9].
* **[ Single Table Inconsistency Threshold ]**: You can select no limit or enter a custom threshold. Validation tasks will be aborted once the threshold is reached.
* **[ Large Table ]**: Large table split concurrency: the maximum number of child split tables that can run simultaneously, with a default value of 10 and a range of [2,15]; large table split row count: the number of rows that can be considered as a child split item, with a default value of 50000000 and a range of [1,80000000].
* **[ LOB Settings ]**: 
  *  After enabling the object to only check for null values, when the object types of the source and target ends are "CLOB", "NCLOB", "BLOB", "LONG", and "LONG RAW", you can only check whether they are NULL without performing content verification.
  *  After disabling "Check Only for Null Objects", the one-time fetch length for LOB types: the size of the LOB read at one time, the default value is 1080, and the value range is [108, 2160]; the difference display length for inconsistent LOB types: the sum of the offset and LOB content displayed when there is a difference in LOBs, the default value is 200, the value range is [1, 400], and this value must be less than the one-time fetch length of the LOB type.
* **[ Character Type Options ]**: Trimming trailing spaces for CHAR data type is enabled by default, while trimming trailing spaces for VARCHAR data type is disabled by default.
...
* **[ Enable Periodic Validation ]**: Periodic validation can be configured by setting a specific time and cron expression, for allowing tasks to perform periodic validation, supporting both normal validation and flashback validation methods.
* **[Number and Boolean compatible validation]**: Supports cross-type consistency verification between integer values and boolean values for columns with the same name that are non-validation sort keys on both the source and target sides.
