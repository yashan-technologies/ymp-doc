The data migration phase primarily implements the migration of metadata and data. Before the migration begins, multiple checks will be conducted. Once the checks pass, the migration will start, and users can view the migration progress and details in real-time.

The data migration of YMP has the following general rules:

- It supports pausing a running migration task, resuming after pausing, terminating after pausing, and displaying the current progress information after pausing.
- For users (Schemas) newly created on the target end during migration, these users (Schemas) will be granted both CONNECT and RESOURCE privileges.

- After the migration is complete, the default password for the DBLINK object will be set to yasdb_123.

  - If there are completely empty rows (discard rows) on the source side that cause the migration to fail, the data file will not be retained. Detailed information about empty row migration failures can be found in the discard.log located in the same directory as the data file (YMP installation directory /tmp/).

  - If the Schema used by the migration task in the starting step is used by other tasks in the selected target database, a prompt will appear when saving or proceeding to the next step: [xxxx] has already been used in task [yyyy] on the target end. Please delete the related task and recreate it. Here, xxxx represents the involved Schema name, and yyyy represents the involved task name.
  - All column names of the target end database table must be included in the source database table, and case sensitivity must be strictly observed; otherwise, the migration will fail.

  - Migration of Schema names or table names that contain single quotes is not supported.
  - Empty LOB (empty_blob() empty_clob()) data will be converted to NULL when migrated to YashanDB.
  - Role migration belongs to the metadata phase one, and is located at the end of metadata phase one. It will be displayed in the object list after the migration is completed. If role migration fails, the privileges of roles that already exist on the target end will be rolled back to the state before the role migration, and newly created roles for the migration task will be deleted.
