The data migration phase mainly implements the migration of metadata and data. Multiple checks will be performed before the migration begins. Once the checks pass, the migration will start, and users can view the migration progress and details in real-time.

YMP's data migration has the following overall rules:

- Support pausing the migration task during execution, support resuming migration after pausing, support terminating migration after pausing, and display current progress information after pausing.
- For users (Schemas) newly created at the target end by the migration task, the user (Schema) will be granted both CONNECT and RESOURCE privileges.

- After migration is completed, the default password for the DBLINK object will be set to yasdb_123.

  - If there are empty rows (discard rows) at the source end causing the migration to fail, the data file will not be retained. The information about the failure of the migration due to empty rows can be found in the discard.log file located in the same directory as the data file (YMP installation directory/tmp/).

  - If the schema used by the migration task in the starting steps is being used by another task in the selected target database, there will be a prompt when saving or proceeding to the next step: [xxxx] has already been used in task [yyyy] at the target end. Please delete the related task and recreate it. Here, xxxx is the schema name involved, and yyyy is the task name involved.

  - Migration of schema names or table names containing single quotes is not supported.
  - Migration of empty LOBs (empty_blob() empty_clob()) is not supported.
